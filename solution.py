...
def validate(data):
    ...
    for record in data:
        ...
        # Kiem tra category khong duoc de trong
    ...
    print(f"Validation summary: {len(valid_records)} valid records kept, {error_count} invalid records dropped.")
...
def transform(data):
    ...
    """
    if not data:
        return pd.DataFrame(columns=['id', 'product', 'price', 'category', 'discounted_price', 'processed_at'])

    df = pd.DataFrame(data)
    
    # Chuyen doi gia tri price sang so thuc
    df['price'] = pd.to_numeric(df['price'], errors='coerce')
    
    # Ap dung các phep bien doi
    df['discounted_price'] = df['price'] * 0.9
